...
public class WidgetCfg extends PreferenceFragment implements OnSharedPreferenceChangeListener {
	...
	private boolean selectList(final boolean lastTry, ProgressDialog dialog) {
		...
		final ProgressDialog currentDialog = dialog == null 
				? ProgressDialog.show(this, "", getResources().getString(R.string.loading), true)
				: dialog;
		...
		try {
			...
			auth.authentificate(this, new AuthentificatedCallback() {
				public boolean authentificated(GoogleAccessProtectedResource protectRes, boolean lt) {			
